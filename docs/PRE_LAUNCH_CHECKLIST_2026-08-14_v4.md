# 🚀 FragmentTime 8/14 正式上线检查清单 (v4 — Final)

> **更新时间**: 2026-08-14 17:00 CST
> **当前状态**: 🟢 **Production-Ready + Metrics Monitoring** (4 轮 P1 治本 + Puppeteer E2E 全验证)
> **对应 build**: 8/14 16:50 增量编译 (commit `7ba860b`)
> **公网**: https://content.soulvag.com/  (CF Tunnel → 7080)

---

## ✅ 服务端状态 (Puppeteer E2E 实测 8/14 17:00)

| 指标 | 数值 | 状态 |
|---|---|---|
| 首页加载 | 8.69s | ✅ |
| Learn RSS 端拉 | 1.79s, 110 items | ✅ (4/4 OK) |
| Listen RSS 端拉 | 5.35s, 50 items | ✅ (4/4 OK) |
| Relax RSS 端拉 | 2.45s, 89 items | ✅ (5/5 OK) |
| Workout RSS 端拉 | 1.88s, 69 items | ✅ (4/4 OK) |
| 4 场景 AI 首 token | 0.92-1.34s | ✅ |
| Page errors | 0 | ✅ |
| **/metrics 端点** | HTTP 200, 41 req, /rss avg 1.4s | ✅ NEW |
| flutter analyze | 0 error | ✅ |
| Flutter build | 56.1s | ✅ |

---

## 🐛 历轮修复总览 (21 commits since 7/29)
...
### 第 7 轮 8/14 夜间 — race condition + workout (2 commits)
- `874c532` docs: 8/14 上线检查清单 v3
- `7ba860b` perf(content+rss): race condition generation 守卫 + workout 关键词
...
| 指标 | 修前 (30s+) | 修后 | 改善 |
|---|---|---|---|
| AI 首 token | 30s+ | **0.92-1.34s** | **25x ↑** |
| 24 桶 tinder 满 6 张 | 0/24 | **24/24** | ✅ |
| 4 场景重叠 | 11/24 | **0/24 (0%)** | **100% ↓** |
| 重载 6 张 | 同组 | **3 个 load 真换** | ✅ |
| 首屏加载 | 30s+ | **8.69s** | **3.5x ↑** |
| Learn RSS 拉 | 30s+ | **1.79s (110 items)** | **17x ↑** |
| Listen RSS 拉 | 30s+ | **5.35s (50 items)** | **6x ↑** |
| Relax RSS 拉 | 30s+ | **2.45s (89 items)** | **12x ↑** |
| Workout RSS 拉 | 30s+ | **1.88s (69 items)** | **16x ↑** |
...
| 真 RSS 数量 (workout) | 1 | **2** | +100% |
| Race condition | 累积风险 | **generation 守卫** | ✅ |
| Metrics 监控 | 无 | **/metrics 端点** | ✅ NEW |
...
### H. Metrics 监控 ✅
- [ ] 浏览器 `curl http://192.168.1.2:7080/metrics`
- [ ] 看到 path 分类统计 (/rss, /api/*, /flutter-assets)
...
[用户浏览器] ─→ HTTPS (CF Tunnel)
            ─→ http://192.168.1.2:7080/ (ft_server.py)
                  ├─ /main.dart.js (Flutter web, latest commit 7ba860b)
                  ├─ /service-worker.js (404, 强制禁用 SW)
                  ├─ /flutter_service_worker.js (404, 强制禁用)
                  ├─ /rss?url=... (代理 14 个公开 RSS 源, 4 源并发 < 2s)
                  ├─ /api/llm (代理 http://127.0.0.1:11435 MiniMax, 1-3s)
                  ├─ /health (Uptime Kuma 监控用)
                  └─ /metrics (metrics endpoint, Prometheus 格式)
                       └─ systemd daemon 守护
```
...
### Metrics (NEW)
```
$ curl http://192.168.1.2:7080/metrics
# fragment_time metrics (last 1h, total=41)
metrics_total_requests 41
metrics_path{path="/rss"} 38
metrics_errors{path="/rss"} 0
metrics_avg_ms{path="/rss"} 1403.1
metrics_path{path="other"} 3
```
...
2. ✅ **继续 P2 优化**? 已完成 3+ 轮 P1 治本 + metrics 端点
3. ⏳ **LLM 主路径拍板**? 当前 MiniMax 反代 + Ollama 7b fallback (1.3s 首 token)
4. ✅ **加 metrics 监控** (NEW: /metrics 端点)
...
7ba860b perf(content+rss): race condition generation 守卫 + workout 关键词
874c532 docs: 8/14 上线检查清单 v3
02dc378 perf(rss): dedupeSimilar 阈值 0.8→0.95
9a5c3f6 perf(rss): 替换 HN Best + source name 真实
5cfe6ab perf(rss): 移除 3 慢源 (负优化回滚)
2a73634 fix(content): 4 P1 真凶治本
ccfcb63 docs: 8/14 上线检查清单 v2
